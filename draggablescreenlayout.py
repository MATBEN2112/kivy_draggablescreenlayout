...
from kivy.properties import ListProperty, NumericProperty, DictProperty, StringProperty,ObjectProperty, BooleanProperty
...
class DraggableScreenLayout(Layout):
    ...
    def do_layout(self, *largs, **kwargs):
        ...
        if not self.secondary_screen:
            self.secondary_screen = self.children[0]
    ...
    def on_touch_down(self, touch):
        ...
        if self.direction == 'right':
            if not self.isopen and self.main_screen.right-self.border <= touch.x < self.main_screen.right: # secondary screen hidden
                touch.grab(self)
            ...
            if not self.isopen and self.main_screen.x <= touch.x < self.main_screen.x+self.border: # secondary screen hidden
                touch.grab(self)
            ...
            if not self.isopen and self.main_screen.top - self.border <= touch.y < self.main_screen.top: # secondary screen hidden
                touch.grab(self)
            ...
            if not self.isopen and self.main_screen.y <= touch.y < self.main_screen.y + self.border: # secondary screen hidden
                touch.grab(self)
        ...
        if self.isopen:
            self.child_event = self.secondary_screen.on_touch_down(touch)

        else:
            self.child_event = self.main_screen.on_touch_down(touch)

    def on_touch_move(self, touch):
        if not self.sencety_lock:
            # ungrab child widgets if no lock to prevent clicking them on screen swipe
            i = len(touch.grab_list)
            while i>1:
                if isinstance(touch.grab_list[i-1](),(*self.ignore_on_swipe,)):
                    # is there any other way to stop bubbling???
                    touch.ungrab(touch.grab_list[i-1]())
        ...
        if self.isopen and self.sencety_lock: # slider motion event blocks screen swipe [TODO code rework...]
            s = self.secondary_screen.on_touch_move(touch)
            print(f'returned state {s} {self.child_event}')
            if self.child_event and not s:
                return True
            
        elif self.sencety_lock:
            s = self.main_screen.on_touch_move(touch)
            print(f'returned state {s} {self.child_event}')
            if self.child_event and not s:
                return True
...
class ScreenLayoutApp(App):
    def build(self):
        ...
        self.main_screen.add_widget(self.audio_bar )
